...
public interface AttendeeJPARepository extends JpaRepository<Attendee, String>, AttendeeRepository {
  ...
  @Query(value = "SELECT a.id, a.name, a.email, a.created_at, ch.created_at as checked_in_at FROM attendees a LEFT JOIN checkins ch ON ch.attendee_id = a.id WHERE a.event_id = ?1", nativeQuery = true)
